...
for k in range(1, imu_f.data.shape[0]):  # start at 1 b/c we have initial prediction from gt
    ...
    p_check = p_est[k-1] + delta_t * v_est[k-1] + delta_t**2 / 2 * (C_ns_dot_fk_1 + g)
    ...
    if gnss_i < gnss.data.shape[0] and gnss.t[gnss_i] <= imu_f.t[k]:
        ...
        sensor_var = var_gnss
        p_check, v_check, q_check, p_cov_check = measurement_update(sensor_var, p_cov_check, y_k, p_check, v_check, q_check)
        gnss_i += 1
